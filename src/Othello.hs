...
updateState :: Event -> State -> Either Error State
updateState e s =
  case e of
    (Turn Black) ->
      if turn s == White
        then Right s {turn = Black}
        else Left TurnError
    (Turn White) ->
      if turn s == Black
        then Right s {turn = White}
        else Left TurnError
    (Move' l p) ->
      if validMove (Move l p) (board s)
        then Right s {board = board'}
        else Left MoveError
      where board' = updateBoard (Move l p) (board s)

searchDirections :: [Direction]
searchDirections = filter (/= (0, 0)) [(x, y) | x <- [-1 .. 1], y <- [-1 .. 1]]

validMove :: Move -> Board -> Bool
validMove move board =
  let Move location player = move
  ...
  in freePosition location board && or validPaths
...
freePosition l b =
  case getSquareByLocation l b of
...
updateBoard move board =
  ...
  in placeSquares squares board

placeSquares :: [Square] -> Board -> Board
placeSquares ss = (map . map) (replaceSquare ss)
  where
    replaceSquare [] s = s
...
getSquareByLocation l board = find findSquareOnLocation (concat board)
  where
    findSquareOnLocation (Square l' _) = l == l'
